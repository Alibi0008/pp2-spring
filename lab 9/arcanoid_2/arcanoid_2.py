...
bonus_blocks = bonus_bricks()  # Initialize bonus blocks
...
def main_menu():
    ...
    start_rect = start.get_rect(center=(W // 2, H // 2 - 50))
    screen.blit(start, start_rect)
    
    # Settings button
    settings = font.render('Settings', True, (255, 255, 255))
    settings_rect = settings.get_rect(center=(W // 2, H // 2))
    screen.blit(settings, settings_rect)
    
    # Quit button
    quit = font.render('Quit', True, (255, 255, 255))
    quit_rect = quit.get_rect(center=(W // 2, H // 2 + 50))
    screen.blit(quit, quit_rect)
    
    pygame.display.flip()
    
    return click(start_rect, settings_rect, quit_rect)

# Function to wait for mouse click to start game
def click(start_rect, settings_rect, quit_rect):
    while True:
...
# Run main menu before game starts
menu = main_menu()
...
if menu == 'start':
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    paused = not paused  # Toggle pause state when spacebar is pressed
        ...
        else:  # If game is paused, show settings menu
            menu = main_menu()
